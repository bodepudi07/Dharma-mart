...
export const FloatingDock = () => {
    ...
    // Only 3 core items for the exclusive experience
    const items = [
        { icon: 'home', label: 'Home', action: () => navigate('home') },
        { icon: 'temple', label: 'Darshans', action: () => navigate('temples') },
        { icon: 'om', label: 'Chanting', action: () => navigate('chantingZone') },
    ];

    if (!currentUser) return null; // Don't show dock if not authenticated
